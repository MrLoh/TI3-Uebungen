...
#define PMAX 5 //Anzahl der Prozesse
#define TINT 50 //Zeitintervall zwischen den Schritten des Algorithmus in Mikrosekunden
#define PDOTS 0//Anzahl der Punkte fuer die Anzeige eines Intervalls
...
int main(void)
{
    ...
    int processes[PMAX][2]={{0,3},{2,6},{4,4},{6,5},{8,2}};
...
// Diese Methode ist fuer das eigentliche schedulen einr Methode zustaendig die die angegebene Signatur hat.Als Argumente erhaelt sie einen Zeiger auf die verkettete Prozessliste,die auszufuehrenden Prozess und einen Zeiger auf die auszufuehrende Funktion
void schedule(LINK head,int processes[PMAX][2], LINK (*sAlgo) (LINK,LINK,int))
{
    LINK current=head;
    int nextPrcs=0;
    ...
    // Fuehre das scheduling durch,solange noch Prozesse kommen werden oder noch Prozesse laufen
...
// Diese Methode fuehrt den Prozess current aus indem die serviceTime dekrementiert wird und die restlichen Elemente aktualisiert werden
void updateProcess(LINK current,int tStep)
{
...
{
    int i;
    char *out=(char*)malloc(8*sizeof(char)*current->pId+10) ;
    strcpy(out,"    ");
